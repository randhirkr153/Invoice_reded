  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (response.data.data.user.role !== 'client') {
        toast.error('Access denied. Client hub only.');
        dispatch(loginFail('Unauthorized'));
        return;
      }
      
      dispatch(loginSuccess(response.data.data));
      toast.success('Welcome back to your billing dashboard!');
      navigate('/');
    } catch (error) {
      ...
      <div className="w-full max-w-md p-8 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl card-shadow">
        ...
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
            Sign in to check invoices, payment receipts, and billing history
          </p>
        </div>
